...
public class EaseConversationAdapter extends ArrayAdapter<EMConversation> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (conversation.getType() == EMConversationType.GroupChat) {
            ...
            Log.i("999999999999999999",username+"====="+EaseGlobal.groupList.size());
            if(EaseGlobal.groupList.size()>0){
                for(EaseAllGroup ep:EaseGlobal.groupList){
                    if(username.equals(ep.group_hxid)){
                        holder.avatar.setImageBitmap(ep.group_bitmap);
                        holder.name.setText(ep.group_nickname);
                        break;
                    }
                }
            ...
            if(EaseGlobal.memberList.size()>0){
                    for(EaseMember em : EaseGlobal.memberList){
                        if(em.member_hxid.equals(username)){
                            holder.avatar.setImageBitmap(em.bitmap);
                            holder.name.setText(em.member_nickname);
                            break;
                        }
                    }
            }else {
